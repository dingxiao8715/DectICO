...
for i_select=1: parameter_length
    ...
    creat_perl=perl ('creatsh_first.pl',filename,Filename_txt{ i_select },Filename_cv{ i_select },vector_number1,vector_number2,Modelname_txt{ i_select });
    fid=fopen('n.sh','at+');
    fprintf(fid,'%s\n',creat_perl);
...
creat_perl=perl ('creatsh_second.pl');
fid=fopen('n.sh','at+');
fprintf(fid,'%s\n',creat_perl);

%% end======================================
fclose('all');
system('rm feature_*.pl');
system('rm fea.txt');
clear all;
%% do svm======================================
system ('chmod 777 n.sh');
system ('./n.sh');
clear all;
clc;
system('rm pls.mat');
